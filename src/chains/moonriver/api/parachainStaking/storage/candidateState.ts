...
export const CollatorState2 = {
    async getMany(
        ...
        if (!pscs2s.isExists) return CollatorState.getMany(ctx, block, addresses)

        let states: (ParachainStaking.CandidateState | undefined)[] = []
        if (pscs2s.isV53) {
            const r = await pscs2s.asV53.getMany(addresses)
            for (let state of r) {
                states.push(
                    state == null
                        ? undefined
...
export const CollatorState = {
    async getMany(
        ...
        const pscss = new ParachainStakingCollatorStateStorage(ctx, block)
        if (!pscss.isExists) return undefined

        let states: (ParachainStaking.CandidateState | undefined)[] = []
        if (pscss.isV49) {
            const r = await pscss.asV49.getMany(addresses)
            for (let state of r) {
                states.push(
                    state == null
                        ? undefined
                        : {
                              bond: state.bond,
                              topDelegations: state.nominators,
                              bottomDelegations: [],
                          }
                )
            }
